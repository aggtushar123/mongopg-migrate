...
| Discriminator-filtered mappings (`filter: {field, equals}`, PRD §7 P0 — the last previously-unbuilt P0): closes the loop from detection (already built) to an actual multiple-mappings-per-collection capability | ✅ — live-tested: one polymorphic collection split into two entities/tables, correctly-scoped counts and dry-run checks per filter. Known follow-on gap: `validate-mapping`'s unmapped-field check introspects the whole (unfiltered) collection, so a field that only exists on a *different* discriminator value can show up as a false "no disposition" — `migrate`/`dry-run`/`validate` are unaffected, this is `validate-mapping`-only |
